...
if [[ ${#pulsarTopics[@]} -gt 0 ]]; then
    ...
    if [[ -f "${scnCfgHomeDir}/${schemaCfgJson}" ]]; then
        ...
        outputMsg "[WARN] Skip schema creation due to invalid config json file!" 4 ${deployMainLogFile} true 
    fi
fi
...
if [[ ${#pulsarFunctions[@]} -gt 0 ]]; then
    ...
    for funcFullName in "${pulsarFunctions[@]}"; do
        ...
        funcCfgJsonFile="${scnCfgHomeDir}/${funcCoreName}.json"
        outputMsg "funciton config file : ${funcCfgJsonFile}" 6 ${deployMainLogFile} true
